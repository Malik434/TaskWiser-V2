...
import { Loader2, Upload, User, Sparkles, Check, X, Award } from "lucide-react"
...
export function ProfileSetupForm({ isOpen, onClose }: ProfileSetupFormProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="overflow-hidden rounded-[32px] border-slate-200 bg-white/80 p-0 shadow-[0_20px_80px_rgba(99,102,241,0.15)] backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-[0_20px_80px_rgba(99,102,241,0.3)] sm:max-w-2xl">
        {/* Header with gradient accent */}
        <div className="relative overflow-hidden border-b border-slate-200 bg-gradient-to-r from-indigo-50 via-purple-50 to-fuchsia-50 p-8 dark:border-slate-800 dark:from-indigo-950/50 dark:via-purple-950/50 dark:to-fuchsia-950/50">
          ...
          <div className="relative space-y-4 text-center">
            {/* Icon */}
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 via-purple-500 to-fuchsia-500 shadow-[0_10px_40px_rgba(99,102,241,0.4)]">
              <User className="h-8 w-8 text-white" />
            </div>

            {/* Title */}
            <div className="space-y-2">
              <DialogTitle className="text-3xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">
                Set Up Your Profile
              </DialogTitle>
            ...
            {/* Badge */}
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white/60 px-4 py-1.5 text-sm text-slate-700 backdrop-blur dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-8">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input
                ...
                className={`h-11 rounded-xl border-slate-300 pl-4 pr-10 dark:border-slate-700 ${
          ...
          <div className="space-y-3">
              ...
              {SPECIALTY_OPTIONS.map((opt) => {
                ...
                return (
                  <button
                    ...
                    disabled={isLoading}
                    className={`group flex items-center gap-2 rounded-full border-2 px-4 py-2 text-sm font-medium transition-all hover:scale-105 disabled:opacity-50 ${
                      isSelected
            ...
            {specialties.length > 0 && (
              <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400">
                ...
                <span>{specialties.length} specialty{specialties.length !== 1 ? "ies" : ""} selected</span>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            <Button
              ...
              className="h-12 gap-2 rounded-full bg-gradient-to-r from-indigo-500 via-purple-500 to-fuchsia-500 px-8 font-semibold text-white shadow-[0_10px_40px_rgba(99,102,241,0.4)] transition-all hover:scale-[1.02] hover:shadow-[0_15px_50px_rgba(99,102,241,0.5)] disabled:opacity-50 disabled:hover:scale-100"
            >
              {isLoading ? (
                <>
